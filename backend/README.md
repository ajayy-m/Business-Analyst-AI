...
`sample_sales_churn_demo.csv` is a separate dataset for the churn
model specifically. It's worth understanding *why* it's separate:
customer IDs in `sample_sales.csv` were assigned randomly per order,
with no persistent behavior per customer -- so there's genuinely
nothing for a churn model to learn there (confirmed: ROC AUC 0.49,
no better than chance, and that's the *correct*, honest result for
data with no real signal). `sample_sales_churn_demo.csv` instead gives
~22% of customers a real declining-engagement pattern across quarters
before going quiet, and the other ~78% stay steady. Same exact model
code, run unchanged against this dataset, scores ROC AUC 0.738 --
proof the pipeline finds real signal when it exists, rather than the
first result being a bug.
...
| `POST /datasets/{id}/churn` | Trains + evaluates a churn model, returns at-risk customer list |
...
**Churn/at-risk scoring** (`analytics/churn.py`) is the one genuinely
*trained* supervised model in the project — forecasting is a linear
fit and anomaly detection is a z-score threshold, neither is fit/predict
ML. Trains a logistic regression on RFM-style features (recency,
frequency, monetary value) with a proper time-based train/test split:
train on earlier period-to-period transitions, evaluate on a held-out
later transition the model never saw during training — no leakage.
Retrains on all data for the final deployed model, then scores current
customers by churn probability.
...
- ✅ Churn/at-risk customer classification (logistic regression, proper
  time-based train/test split) — see `sample_sales_churn_demo.csv` below
...
- **Customer segmentation (RFM/clustering)** — evaluated and
  deliberately deferred: without a real "current place to plug it in"
  (the diagnostic engine already discovers pre-existing category
  columns like `segment`), building it in isolation would be closer to
  "called `sklearn.cluster.KMeans`" than a capability the product
  actually uses. Worth revisiting if a future dataset has no
  pre-existing segment labels and needs one discovered.
